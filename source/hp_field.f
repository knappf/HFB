     ...
     &              *rhon_HFB(l,k)*dble(2*Jp+1)/dble(levp(i)%j2+1)
            if(if_self.eq.1) then
             val=val+VpY(i,k,j,l,Jp)
     & *rhoY_HFB(l,k)*dble(2*Jp+1)/(dble((levp(i)%j2+1)*(levY(k)%j2+1)))
            endif
            enddo
           endif
          enddo
       ...
       if(Ni+Nk.le.noscmax12.and.Nj+Nm.le.noscmax12) then
          ...
          do indln=1,iprho
           ...
           il=lev1pnm(l)%jsch
            ...
            r4=rhop_HFB(lp1(im),lp1(ik))*rhoY_HFB(lp1(inn),lp1(il))
            r5=rhon_HFB(lp1(im),lp1(ik))*rhoY_HFB(lp1(inn),lp1(il))
...
!****************************
             ...
             enddo
            enddo
            val=val+0.5d0*Vppp*r1+0.5d0*Vpnn*r2+Vppn*r3+Vppl*r4+Vpnl*r5
            endif
           enddo
            endif
           enddo
